...
Requires models that make use of 'preprocess_mixup.py'.
"""
...
def init_mixup(y, alpha=0.1):
    ...
    batch_size = y.size()[0]
    perm_index = torch.randperm(batch_size).cuda()

    # Create permuted labels
    y, y_permuted = y, y[perm_index]

    return (y, y_permuted, lam, perm_index)
...
def train_batches(
    ...
    for i, sample in enumerate(
        ...
        labels, labels_permuted, lam, perm_index = init_mixup(labels, ALPHA)
