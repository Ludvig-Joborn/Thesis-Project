...
class R_Conv_cbam_avg_pool(nn.Module):
    def __init__(self, inC, outC, p_ks=(3, 1), p_stride=(2, 1), p_padding=0):
        super(R_Conv_cbam_avg_pool, self).__init__()
        self.block = nn.Sequential(
            R_Conv(inC=inC, outC=outC),
            CBAM(inC=outC),
            nn.AvgPool2d(kernel_size=p_ks, stride=p_stride, padding=p_padding),
        )
...
class NeuralNetwork(nn.Module):
    def __init__(self, input_sample_rate: int, output_sample_rate: int = SAMPLE_RATE):
        ...
        self.r_conv_cbam3 = R_Conv_cbam_avg_pool(
            inC=32, outC=64, p_ks=(3, 1), p_stride=(2, 1), p_padding=(1, 0)
        )

        # 8/32, 64, 16, 431/157
        self.r_conv_cbam4 = R_Conv_cbam_avg_pool(
            inC=64, outC=128, p_ks=(3, 1), p_stride=(2, 1), p_padding=(1, 0)
        )

        # 8/32, 128, 8, 431/157
        self.r_conv_cbam5 = R_Conv_cbam_avg_pool(
            inC=128, outC=128, p_ks=(3, 1), p_stride=(2, 1), p_padding=(1, 0)
        )
