...
# NOTE: Added to ignore pandas append Future Warning
import warnings
...
RatesPerClass = namedtuple(
    "RatesPerClass", ["tp_ratio", "fp_rate", "ct_rate", "effective_fp_rate", "id"]
)
PSDROC = namedtuple("PSDROC", ["yp", "xp", "mean", "std"])
PSDS = namedtuple(
    "PSDS", ["value", "plt", "alpha_st", "alpha_ct", "max_efpr", "duration_unit"]
)
...
class PSDSEval:
    ...
    def __init__(
        self, dtc_threshold=0.5, gtc_threshold=0.5, cttc_threshold=0.3, **kwargs
    ):
        ...
        self.threshold = Thresholds(
            dtc=dtc_threshold, gtc=gtc_threshold, cttc=cttc_threshold
        )
    ...
    def _validate_simple_dataframe(df, columns, name, allow_empty=False):
        ...
        if not isinstance(df, pd.DataFrame):
            raise PSDSEvalError(
                f"The {name} data must be provided in " "a pandas.DataFrame"
            )
        if not set(columns).issubset(set(df.columns)):
            raise PSDSEvalError(
                f"The {name} data columns need to match the " "following", columns
            )
    ...
    def _validate_input_table_with_events(self, df, name):
        ...
        """
        self._validate_simple_dataframe(df, self.detection_cols, name, allow_empty=True)
        if not df.empty:
            if not df[df.onset > df.offset].empty:
                raise PSDSEvalError(
                    f"The {name} dataframe provided has " f"events with onset > offset."
                )
            ...
            if not intersections[intersections.same_cls].empty:
                raise PSDSEvalError(
                    f"The {name} dataframe provided has "
    ...
    def set_ground_truth(self, gt_t, meta_t):
        ...
        if self.ground_truth is not None or self.metadata is not None:
            raise PSDSEvalError(
                "You cannot set the ground truth more than" " once per evaluation"
            )
        ...
        if meta_t is None and gt_t is not None:
            raise PSDSEvalError(
                "Audio metadata is required when adding " "ground truths"
            )
        ...
        _ground_truth = gt_t[self.detection_cols].reset_index(inplace=False, drop=True)
        ...
        _ground_truth = self._update_world_detections(
            self.detection_cols, _ground_truth, metadata_t
        )
        # remove zero-length events
        _ground_truth = _ground_truth[_ground_truth.offset > _ground_truth.onset]
        ground_truth_t = _ground_truth.sort_values(by=self.detection_cols[:2], axis=0)
        ground_truth_t.dropna(inplace=True)
        ground_truth_t["duration"] = ground_truth_t.offset - ground_truth_t.onset
    ...
    def _init_det_table(self, det_t):
        ...
        """
        self._validate_input_table_with_events(det_t, "detection")
        ...
        world_gt = [
            {
                ...
                for k, v in zip(
                    columns,
                    [
                        metadata.loc[i, "filename"],
                        0.0,
                        metadata.loc[i, "duration"],
                        WORLD,
                    ],
                )
            }
    ...
    def _operating_point_id(self, detection_table):
        ...
        detection_table_col_sorted = detection_table[table_columns]
        detection_table_row_sorted = detection_table_col_sorted.sort_values(
            by=table_columns
        )
        h = hashlib.sha256(
            pd.util.hash_pandas_object(detection_table_row_sorted, index=False).values
        )
        uid = h.hexdigest()
    ...
    @staticmethod
    def _get_table_intersections(
        table1, table2, suffixes=("_1", "_2"), remove_identical=False
    ):
        ...
        Returns:
            ...
            considered.
        """
        comb_t = pd.merge(table1, table2, how="outer", on="filename", suffixes=suffixes)
    ...
    def _ground_truth_intersections(self, detection_t, ground_truth_t):
        ...
        cross_t["inter_duration"] = np.minimum(
            cross_t.offset_det, cross_t.offset_gt
    ...
    def _detection_and_ground_truth_criteria(self, cross_t):
        ...
        if gt_cross_t.empty:
            dtc_t = pd.DataFrame(columns=["id_det", "event_label_gt", "det_precision"])
        else:
            dtc_t = (
                gt_cross_t.groupby(["id_det", "event_label_gt"])
                .det_precision.sum()
                .reset_index()
            )
        ...
        gtc_t = (
            gt_cross_t[gt_cross_t.id_det.isin(dtc_ids)]
            .groupby(["id_gt", "event_label_det"])
            .gt_coverage.sum()
            .reset_index()
        )
        ...
        if len(dtc_t) or len(gtc_t):
            tmp = pd.merge(
                ...
                suffixes=("", "_sum"),
            ).merge(gtc_t, on=["id_gt", "event_label_det"], suffixes=("", "_sum"))
        else:
    ...
    def _confusion_matrix_and_rates(self, tp, ct):
        ...
        ct_tmp = ct.groupby(["event_label_det", "event_label_gt"]).filename.count()
    ...
    def _cross_trigger_criterion(self, inter_t, tp_t, dtc_ids):
        ...
        ct_t = inter_t[
            ~inter_t.id_det.isin(tp_t.id_det)
            & ~inter_t.same_cls
            & ~inter_t.id_det.isin(dtc_ids)
        ]
        ...
        cttc = ct_t[
            (ct_t.det_precision_sum >= self.threshold.cttc)
    ...
    def _evaluate_detections(self, det_t):
        ...
        cts, tp_ratio, fp_rate, ct_rate = self._confusion_matrix_and_rates(tp, cttc)

        return cts, tp_ratio, fp_rate, ct_rate  # NOTE
    ...
    def add_operating_point(self, detections, info=None):
        ...
        if self.ground_truth is None:
            raise PSDSEvalError(
                "Ground Truth must be provided before "
                "adding the first operating point"
            )
        if self.metadata is None:
            raise PSDSEvalError(
                "Audio metadata must be provided before "
                "adding the first operating point"
            )
        ...
        self._add_op(
            opid=op_id, counts=cts, tpr=tp_ratio, fpr=fp_rate, ctr=ct_rate, info=info
        )
    ...
    def _add_op(self, opid, counts, tpr, fpr, ctr, info=None):
        ...
        if set(op.keys()).isdisjoint(set(info.keys())):
            op.update(info)
            self.operating_points = self.operating_points.append(op, ignore_index=True)

        else:
            raise PSDSEvalError(
                "the 'info' cannot contain the keys 'id', "
    ...
    def step_curve(x, xp, yp):
        ...
        roc_valid_only = (
            roc_orig.groupby("x").agg("max").reset_index().sort_values(by="x")
        )
        ...
        roc_new = (
            pd.merge(pd.Series(x, name="x"), roc_valid_only, how="outer", on="x")
            .fillna(method="ffill")
            .fillna(value=0)
        )
        return roc_new.y.values

    def _effective_fp_rate(self, alpha_ct=0.0):
        ...
        tpr_arr = np.pad(tpr_arr, ((0, 0), (0, 1)), "constant", constant_values=0)
        fpr_arr = np.stack(self.operating_points.fpr.values, axis=1)
        fpr_arr = np.pad(fpr_arr, ((0, 0), (0, 1)), "constant", constant_values=0)
        ctr_arr = np.stack(self.operating_points.ctr.values, axis=2)
        ctr_arr = np.pad(
            ctr_arr, ((0, 0), (0, 0), (0, 1)), "constant", constant_values=0
        )
        id_arr = self.operating_points.id.values
        id_arr = np.pad(id_arr, ((0, 1),), "constant", constant_values="None")

        efpr = fpr_arr  # + alpha_ct * np.nanmean(ctr_arr, axis=1)  # NOTE: Code modified here

        return RatesPerClass(
            tp_ratio=tpr_arr,
            fp_rate=fpr_arr,
            ct_rate=ctr_arr,
            effective_fp_rate=efpr,
            id=id_arr,
        )
    ...
    def psd_roc_curves(self, alpha_ct, linear_interp=False):
        ...
        for c in range(n_classes):
            ...
            tpr_v_efpr[c] = _curve(
                efpr_points, pcr.effective_fp_rate[c], pcr.tp_ratio[c]
            )
            for k in range(n_classes):
                if c == k:
                    continue
                tpr_v_ctr[c, k] = _curve(ctr_points, pcr.ct_rate[c, k], pcr.tp_ratio[c])
        ...
        tpr_vs_ctr_c = PSDROC(
            # yp=tpr_v_ctr,  # NOTE: Code modified here
            # xp=ctr_points,
    ...
    def compute_f_score(TP_values, FP_values, FN_values, beta):
        ...
        k = 1 + beta**2
        f_scores = (k * TP_values) / (
            k * TP_values + (beta**2) * FN_values + FP_values
        )
        return f_scores

    def compute_macro_f_score(self, detections, beta=1.0):
        ...
        f_per_class = self.compute_f_score(
            per_class_tp, per_class_fp, per_class_fn, beta
        )
    ...
    def select_operating_points_per_class(
        self, class_constraints, alpha_ct=0.0, beta=1.0
    ):
        ...
        for index, row in class_constraints.iterrows():
            ...
            if constraint == "tpr":
                ...
                raise PSDSEvalError(
                    f"The constraint has to be one of tpr, "
            ...
            if (op_index is None) or (pcr.id[_filter][op_index] == "None"):
                ...
                # NOTE: Code modified here
                chosen_op_point_dict.update(
                    chosen_op_point.to_dict(orient="records")  # [0]
                )
                # NOTE: Code added here
                chosen_op_point_dict["threshold"] = chosen_op_point.threshold.values[0]

            _op_points_t = _op_points_t.append(chosen_op_point_dict, ignore_index=True)
    ...
    def _effective_tp_ratio(tpr_efpr, alpha_st):
        ...
        return PSDROC(xp=tpr_efpr.xp, yp=etpr, std=tpr_efpr.std, mean=tpr_efpr.mean)
    ...
    def psds(self, alpha_ct=0.0, alpha_st=0.0, max_efpr=None, en_interp=False):
        ...
        tpr_fpr_curve, tpr_ctr_curve, tpr_efpr_curve = self.psd_roc_curves(
            alpha_ct, en_interp
        )
        ...
        score = self._auc(psd_roc.xp, psd_roc.yp, max_efpr, alpha_st > 0) / max_efpr
        return PSDS(
            value=score,
            plt=psd_roc,
            alpha_st=alpha_st,
    ...
    def _auc(x, y, max_x=None, decreasing_y=False):
        ...
        if x.size != y.size:
            raise PSDSEvalError(
                f"x and y must be of equal " f"length {x.size} != {y.size}"
            )
...
def plot_psd_roc(psd, en_std=False, axes=None, filename=None, **kwargs):
    ...
    if en_std:
        ...
        axes.step(psd.plt.xp, psd.plt.mean + psd.plt.std, c="b", linestyle="--")
    axes.fill_between(
        psd.plt.xp,
        y1=psd.plt.yp,
        y2=0,
...
def plot_per_class_psd_roc(
    psd, class_names, max_efpr=None, axes=None, filename=None, **kwargs
):
    """
    ...
    axes.step(psd.xp, psd.mean, lw=2.0, ls="--", label="mean_TPR", where="post")
